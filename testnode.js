...
/* --------------- don,t have a wall sign in part--------------------------------------- */
      ...
      tests.collection('bbw_users').find({u_name:username,pswd:password}).toArray(function(err, res){
        wallId = res[0].wallId;
        console.log(wallId);
        console.log(socket.id);
         if(!res.length == 0)
          {
          var response = {'status':1,'wallId':wallId};
          socket.emit('sigin',response);
            /* ------------- finding the links from aws server for signin -----------*/
            tests.collection('imagecollection').find({wall_id:wallid}).toArray(function(err, res){
            var response = {im_1:res[0].img_1,im_2:res[0].img_2,im_3:res[0].img_3,im_4:res[0].img_4,im_5:res[0].img_5,im_6:res[0].img_6,im_7:res[0].img_7,im_8:res[0].img_8,im_9:res[0].img_9};
